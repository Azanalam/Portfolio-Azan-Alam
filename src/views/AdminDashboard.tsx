...
export default function AdminDashboard() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-(--bg) text-(--text-1) flex flex-col items-center justify-center font-mono text-xs gap-3">
  ...
  return (
    <div className="min-h-screen bg-(--bg) text-(--text-1) font-sans flex">
      {/* Side Control Rail */}
      <aside className="w-64 bg-(--surface) border-r border-(--line) hidden md:flex flex-col justify-between shrink-0 h-screen sticky top-0">
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-3 border-b border-(--line) pb-5">
            ...
            <div>
              <h1 className="text-sm font-bold tracking-tight text-(--text-1)">CMS Controller</h1>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 text-xs font-bold rounded-lg transition-colors cursor-pointer text-left ${
                    activeTab === item.id
                      ...
                      : 'text-(--text-muted) hover:text-(--text-1) hover:bg-(--surface-3)/50 border border-transparent'
                  }`}
                >
        ...
        {/* User context footer */}
        <div className="p-6 border-t border-(--line) space-y-3.5">
          <div className="flex items-center gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-xs text-(--text-1) font-bold truncate">{username}</p>
              <p className="text-[9px] font-mono text-(--text-faint) uppercase">Administrator</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={() => navigate('/')}
              className="w-full inline-flex items-center justify-center gap-1.5 py-1.5 bg-(--surface-2) hover:bg-(--surface-3) text-(--text-mid) hover:text-(--text-1) border border-(--line) rounded text-[10px] uppercase font-bold tracking-wider transition cursor-pointer"
            >
      ...
      <main className="flex-grow min-w-0 p-6 md:p-10 space-y-8 overflow-y-auto max-h-screen">
        ...
        <div className="flex md:hidden items-center justify-between bg-(--surface) border border-(--line) p-4 rounded-xl mb-4">
          <div className="flex items-center gap-2">
            <Shield className="h-4 w-4 text-emerald-400" />
            <span className="text-xs font-mono uppercase tracking-wider text-(--text-1)">CMS controller</span>
          </div>
          ...
          <div className="flex gap-2">
            <select
              ...
              className="bg-(--surface-2) border border-(--line) text-(--text-1) text-[11px] px-2 py-1 rounded focus:outline-none"
            >
              {navItems.map((item) => (
            ...
            {activeTab === 'overview' && db && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                  <div className="bg-(--surface) border border-(--line) hover:border-(--line-strong) transition-colors p-5 rounded-xl space-y-2">
                    <span className="text-[9px] font-mono font-bold uppercase text-(--text-faint)">CASE_STUDIES_COUNT</span>
                    <p className="text-2xl font-bold text-(--text-1) font-sans">{db.projects?.length || 0}</p>
                    <span className="text-[10px] text-emerald-400 font-bold block">Dynamic Projects</span>
                  </div>

                  <div className="bg-(--surface) border border-(--line) hover:border-(--line-strong) transition-colors p-5 rounded-xl space-y-2">
                    <span className="text-[9px] font-mono font-bold uppercase text-(--text-faint)">CHRONO_MILESTONES</span>
                    <p className="text-2xl font-bold text-(--text-1) font-sans">{db.experiences?.length || 0}</p>
                    <span className="text-[10px] text-emerald-400 font-bold block">Work Chronologies</span>
                  </div>

                  <div className="bg-(--surface) border border-(--line) hover:border-(--line-strong) transition-colors p-5 rounded-xl space-y-2">
                    <span className="text-[9px] font-mono font-bold uppercase text-(--text-faint)">MATRIX_SKILL_ROWS</span>
                    <p className="text-2xl font-bold text-(--text-1) font-sans">
                  ...
                  <div className="bg-(--surface) border border-(--line) hover:border-(--line-strong) transition-colors p-5 rounded-xl space-y-2">
                    <span className="text-[9px] font-mono font-bold uppercase text-(--text-faint)">SYSTEM_MEDIA_ASSETS</span>
                    <p className="text-2xl font-bold text-(--text-1) font-sans">{db.media?.length || 0}</p>
                    <span className="text-[10px] text-emerald-400 font-bold block">Uploaded media</span>
                  </div>
                </div>

                {/* Information Callout */}
                <div className="p-6 bg-(--surface) border border-(--line) rounded-2xl relative overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-emerald-500 via-transparent to-transparent" />
                  <div className="space-y-1">
                    <h3 className="text-sm font-bold text-(--text-1) font-sans">Administrative CMS Session State</h3>
                    <p className="text-xs text-(--text-muted) leading-relaxed max-w-xl">
                      Welcome, <span className="text-(--text-1) font-bold">{username}</span>. You can edit any section on your portfolio securely from the side navigation control. No source code modifications or redeployments required.
                    </p>
                  </div>
                  <button
                    onClick={() => navigate('/')}
                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-(--cta-bg) hover:bg-(--cta-hover) text-(--cta-fg) text-xs font-bold uppercase tracking-wider rounded-lg transition-colors cursor-pointer font-sans shrink-0"
                  >
