...
export default function Services({ onNavigate }: ServicesProps) {
  ...
  return (
    ...
    >
      ...
      {services.length === 0 ? (
        <div className="p-12 text-center rounded-2xl border border-dashed border-[#1A1A1A] bg-[#0A0A0A]/30">
          <p className="text-xs font-mono text-[#71717A]">No services published yet. Check back soon.</p>
        </div>
      ) : (
            ...
            return (
              <motion.div
                ...
                className={`flex flex-col justify-between p-7 rounded-2xl border transition-all ${
                  service.highlight
                    ? 'bg-[#0A0A0A] border-emerald-500/30 shadow-[0_0_40px_-20px_rgba(16,185,129,0.25)]'
                    : 'bg-[#0A0A0A] border-[#1A1A1A] hover:border-[#27272A]'
                }`}
              >
                <div className="space-y-5">
                  <div className="flex items-start justify-between">
                    <div className="p-3 rounded-xl bg-[#111] border border-[#1A1A1A] text-emerald-400">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-baseline justify-between gap-2">
                      <h2 className="text-lg font-bold text-white tracking-tight">{service.title}</h2>
                  ...
                  <ul className="space-y-2">
                    {service.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-2 text-xs text-[#71717A] font-sans">
                ...
                <div className="pt-5 mt-6 border-t border-[#1A1A1A]">
                  <button
                    onClick={() => onNavigate('/contact')}
                    className="inline-flex items-center gap-2 text-[10px] font-mono font-bold uppercase tracking-wider text-white hover:text-emerald-500 transition cursor-pointer bg-transparent border-none"
                  >
      ...
      <motion.div
        variants={itemVariants}
        className="p-6 rounded-xl border border-[#1A1A1A] bg-[#0A0A0A]/60 flex flex-col sm:flex-row sm:items-center justify-between gap-4"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">Clear process, no surprises</h3>
            <p className="text-xs text-[#71717A] mt-0.5">Scope → Build → Test → Handoff & support.</p>
          </div>
        </div>
        <button
          onClick={() => onNavigate('/contact')}
          className="inline-flex items-center justify-center gap-2 bg-[#FAFAFA] hover:bg-white text-black font-bold text-xs uppercase tracking-wider px-5 py-3 rounded-lg transition cursor-pointer"
        >
