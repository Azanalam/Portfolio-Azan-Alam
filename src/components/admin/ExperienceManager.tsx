  ...
  return (
    <div className="space-y-12 font-sans">
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-(--line) pb-5">
          ...
          {!editingExp && !editingEdu && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-(--cta-fg) font-bold text-xs uppercase tracking-wider rounded-lg transition-colors cursor-pointer"
            >
        ...
        {!editingExp && !editingEdu && (
          <div className="space-y-4">
            {experiences.length === 0 ? (
              <div className="text-center p-12 border border-dashed border-(--line) bg-(--surface)/30 rounded-xl">
                <span className="text-[10px] font-mono text-(--text-faint) block mb-1">NO DATA</span>
                <p className="text-xs text-(--text-muted)">Press button above to add career chronological blocks.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {experiences.map((exp) => (
                  <div
                    key={exp.id}
                    className="p-5 bg-(--surface) border border-(--line) hover:border-(--line-strong) transition rounded-xl flex flex-col justify-between"
                  >
                    <div className="space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-sm font-bold text-(--text-1) tracking-tight">{exp.role}</h4>
                          <span className="text-xs text-emerald-400 font-bold font-mono">{exp.company}</span>
                        </div>
                        <span className="text-[10px] font-mono text-(--text-muted) bg-(--surface-2) border border-(--line) px-2 py-0.5 rounded">
                          {exp.period}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-(--line) mt-4">
                      <span className="text-[9px] font-mono text-(--text-faint)">{exp.location || 'Remote'}</span>
        ...
        {editingExp && (
          <form onSubmit={handleSaveExp} className="bg-(--surface) border border-(--line) rounded-xl p-6 space-y-6">
            <div className="flex justify-between items-center border-b border-(--line) pb-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1.5 bg-(--surface-2) hover:bg-(--surface-3) text-(--text-1) text-[10px] uppercase font-bold tracking-wider rounded"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={saving}
                  className="px-3.5 py-1.5 bg-emerald-500 hover:bg-emerald-400 text-(--cta-fg) text-[10px] uppercase font-bold tracking-wider rounded"
                >
                  Save Milestone
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Entity / Company Name</span>
                <input
                  type="text"
                  required
                  placeholder="SpaceX"
              ...
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Position / Role Title</span>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Timeframe Period</span>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Geographic Location</span>
                <input
                  type="text"
            ...
            <div className="space-y-3">
              <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Milestones & Achievements</span>
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                {editingExp.description.map((desc: string, idx: number) => (
                  <div key={idx} className="flex gap-2 bg-(--surface-2) border border-(--line) p-2 rounded justify-between items-start">
                    <p className="text-[11px] text-(--text-2) leading-relaxed font-sans">{desc}</p>
                    <button
                      type="button"
              ...
              <div className="flex gap-1.5">
                <input
                  ...
                  className="flex-grow bg-(--surface-2) border border-(--line) text-(--text-1) rounded px-2.5 py-1.5 text-xs focus:outline-none font-sans"
                />
                <button
                  type="button"
                  onClick={() => {
            ...
            {/* Technologies list */}
            <div className="space-y-2">
              <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Technologies Utilized</span>
              <div className="flex flex-wrap gap-1">
                {editingExp.tags.map((tech: string) => (
                  <span key={tech} className="inline-flex items-center gap-1 px-2 py-0.5 text-[9px] font-mono text-(--text-1) bg-(--surface-2) border border-(--line) rounded">
                    <span>{tech}</span>
                    <button
                      type="button"
              ...
              <div className="flex gap-1.5 max-w-sm">
                <input
                  ...
                  className="flex-grow bg-(--surface-2) border border-(--line) text-(--text-1) rounded px-2.5 py-1 text-xs focus:outline-none font-mono"
                />
                <button
                  type="button"
                  onClick={() => {
      ...
      <div className="space-y-6 pt-6 border-t border-(--line)">
        ...
        {!editingExp && !editingEdu && (
          <div className="space-y-4">
            {education.length === 0 ? (
              <div className="text-center p-12 border border-dashed border-(--line) bg-(--surface)/30 rounded-xl">
                <span className="text-[10px] font-mono text-(--text-faint) block mb-1">NO DATA</span>
                <p className="text-xs text-(--text-muted)">Press button above to add academic chronological blocks.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {education.map((edu) => (
                  ...
                  >
                    <div className="space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-sm font-bold text-(--text-1) tracking-tight">{edu.degree}</h4>
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-(--line) mt-4">
                      <span className="text-[9px] font-mono text-(--text-faint)">{edu.location}</span>
                      <div className="flex gap-1.5 items-center">
                        <button
                          type="button"
        ...
        {/* Education Edit Form */}
        {editingEdu && (
          <form onSubmit={handleSaveEdu} className="bg-(--surface) border border-(--line) rounded-xl p-6 space-y-6">
            <div className="flex justify-between items-center border-b border-(--line) pb-4">
              <h3 className="text-xs font-mono font-bold uppercase text-emerald-400 tracking-wider">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">University / School</span>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Degree / Course Major</span>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Period Range</span>
                <input
                  type="text"
                  required
            ...
            <div className="space-y-1">
              <span className="text-[9px] font-mono font-bold text-(--text-muted) uppercase block">Degree dissertation details</span>
              <textarea
                ...
                className="w-full bg-(--surface-2) border border-(--line) text-(--text-1) rounded px-2.5 py-1.5 text-xs focus:outline-none resize-none font-sans"
              />
            </div>
          </form>
        )}
