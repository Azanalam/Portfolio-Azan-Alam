      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-(--surface) border border-(--line) rounded-xl p-6 space-y-4">
            <h3 className="text-xs font-bold text-(--text-1) uppercase tracking-wider font-mono border-b border-(--line) pb-2 text-emerald-500 flex items-center gap-2">
              ...
              {skillGroups.map((group, idx) => (
                <div
                  ...
                  className={`flex items-center justify-between p-3 border rounded-xl transition cursor-pointer font-mono text-xs ${
                    activeGroupIdx === idx
                      ? 'bg-emerald-950/25 border-emerald-500/40 text-(--text-1) shadow-sm'
                      : 'bg-(--surface-2) border-(--line) text-(--text-mid) hover:border-(--line-strong)'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-(--text-faint)">#{idx + 1}</span>
                    <span className="font-bold">{group.category}</span>
                    <span className="text-[9px] text-(--text-faint)">({group.skills?.length || 0})</span>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      disabled={idx === 0}
                      className="p-0.5 text-(--text-faint) hover:text-(--text-1) transition disabled:opacity-10 cursor-pointer"
                    >
                      ▲
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="p-0.5 text-(--text-faint) hover:text-red-400 transition cursor-pointer"
                    >
            ...
            {/* Add Group Subform */}
            <div className="p-3.5 bg-(--surface-2) border border-(--line) rounded-xl space-y-3 mt-4">
              <div className="text-[9px] font-mono font-bold uppercase tracking-widest text-(--text-muted)">Append Category row</div>
              <div className="flex gap-1.5">
                <input
                  ...
                  className="flex-grow bg-(--surface) border border-(--line) text-(--text-1) rounded px-2.5 py-1.5 text-xs focus:outline-none"
                />
                <button
                  type="button"
                  onClick={addGroup}
                  className="px-4 py-1.5 bg-(--cta-bg) hover:bg-(--cta-hover) text-(--cta-fg) font-bold text-[10px] uppercase tracking-wider rounded transition cursor-pointer"
                >
                  Append
                </button>
              </div>
        ...
        <div className="lg:col-span-7">
          <div className="bg-(--surface) border border-(--line) rounded-xl p-6 space-y-4 min-h-[400px] flex flex-col justify-between">
            <div>
              <h3 className="text-xs font-bold text-(--text-1) uppercase tracking-wider font-mono border-b border-(--line) pb-2 text-emerald-500 flex items-center gap-2">
              ...
              {activeGroupIdx === null ? (
                <div className="text-center p-16 font-sans text-xs text-(--text-muted)">
                ...
                <div className="space-y-3 pt-3">
                  {skillGroups[activeGroupIdx].skills?.length === 0 ? (
                    <div className="text-center p-8 font-mono text-[10px] text-(--text-faint)">
                    ...
                    <div className="flex flex-wrap gap-2.5 max-h-80 overflow-y-auto pr-1">
                      {skillGroups[activeGroupIdx].skills.map((skill: any, idx: number) => (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1.5 px-3 py-1 bg-(--surface-2) border border-(--line) rounded-lg font-mono text-xs text-(--text-1)"
                        >
                          <span
                          ...
                          <button
                            ...
                            className="text-(--text-muted) hover:text-red-400 font-bold ml-1.5 text-sm cursor-pointer"
                          >
                            ×
                          </button>
                        </span>
            ...
            {activeGroupIdx !== null && (
              <div className="p-4 bg-(--surface-2) border border-(--line) rounded-xl space-y-3.5 mt-6">
                <div className="text-[9px] font-mono font-bold uppercase tracking-widest text-(--text-muted)">
                  Append skill row to: {skillGroups[activeGroupIdx].category}
                </div>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <span className="text-[8px] font-mono font-bold text-(--text-muted) block uppercase">Skill Label</span>
                    <input
                      type="text"
                      placeholder="e.g. WebAssembly"
                      value={newSkillName}
                      onChange={(e) => setNewSkillName(e.target.value)}
                      className="w-full bg-(--surface) border border-(--line) text-(--text-1) rounded px-2.5 py-1.5 text-xs focus:outline-none"
                    />
                  </div>

                  <div className="space-y-1">
                    <span className="text-[8px] font-mono font-bold text-(--text-muted) block uppercase">Custom Highlight Color</span>
                    <div className="flex gap-2">
                      ...
                      <input
                        ...
                        value={newSkillColor}
                        onChange={(e) => setNewSkillColor(e.target.value)}
                        className="bg-(--surface) border border-(--line) text-(--text-1) px-2 py-1 rounded text-xs focus:outline-none font-mono"
                      />
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full inline-flex items-center justify-center gap-1 py-1.5 bg-(--cta-bg) hover:bg-(--cta-hover) text-(--cta-fg) font-bold text-[10px] uppercase tracking-wider rounded transition cursor-pointer"
                >
